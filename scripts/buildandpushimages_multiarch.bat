...
echo "building and pushing image testgary22-buildstage"
pushd source
IF  "%CONTAINER_RUNTIME%" == "docker" (
    docker buildx build --platform ${PLATFORMS} -f Dockerfile.buildstage  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/testgary22-buildstage . 
) ELSE ( 
    podman manifest create ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/testgary22-buildstage
    podman build --platform ${PLATFORMS} -f Dockerfile.buildstage --manifest ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/testgary22-buildstage .
    podman manifest push ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/testgary22-buildstage
)
popd
echo "building and pushing image testgary22"
pushd source
IF  "%CONTAINER_RUNTIME%" == "docker" (
    docker buildx build --platform ${PLATFORMS} -f Dockerfile  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/testgary22 . 
) ELSE ( 
    podman manifest create ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/testgary22
    podman build --platform ${PLATFORMS} -f Dockerfile --manifest ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/testgary22 .
    podman manifest push ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/testgary22
)
popd
